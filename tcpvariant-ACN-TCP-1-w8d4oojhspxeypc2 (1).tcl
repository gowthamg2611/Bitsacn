...
set record_config [open "$TCP_Name-configuration" w]
...
proc monitor {interval} {
    global FlowNumber tcp ns TCP_Name
    set nowtime [$ns now]

    for {set i 0} {$i < $FlowNumber} {incr i 1} {
        set win [open $TCP_Name-result$i a]
	puts $win "$nowtime [$tcp($i) set cwnd_]"
	close $win
    }
...
for {set i 0} {$i < $FlowNumber} {incr i 1} {
	#setup topology
        set win [open $TCP_Name-result$i w]
	close $win
